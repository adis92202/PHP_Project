...
<div class="flex flex-col items-center">
    ...
    <!--Header-->
    <h2 class="w-1/2 text-center bg-lime-200 text-4xl py-5 font-extrabold rounded-t-lg text-lime-700 border-4 border-lime-200">Adding new game</h2>
        ...
        <form method="POST" action="{{ route('library.store') }}" enctype="multipart/form-data">
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-3">
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" value="{{old('title')}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-lime-300 focus:border-lime-300 block w-full p-2.5">
                </div>
                <!--Category-->
                <div>
                    ...
                    <select id="category" name="category" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-lime-300 focus:border-lime-300 block w-full p-2.5">
                        ...
                        <option value="Cards" {{ old('category') == "Cards" ? 'selected' : '' }}>Cards</option>
                        <option value="Children" {{ old('category') == "Children" ? 'selected' : 'Children' }}>Children</option>
                        <option value="Economic" {{ old('category') == "Economic" ? 'selected' : '' }}>Economic</option>
                        ...
                        <option value="Humor" {{ old('category') == "Humor" ? 'selected' : '' }}>Humor</option>
                        <option value="Murder/Mystery" {{ old('category') == "Murder/Mystery" ? 'selected' : '' }}>Murder/Mystery</option>
                ...
                <div>
                    ...
                    <input type="number" id="year" name="year" value="{{old('year')}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-lime-300 focus:border-lime-300 block w-full p-2.5">
                </div>
            </div>
            <div class="grid gap-6 mb-6 md:grid-cols-3">
                <!--Adding Image-->
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="number" id="time" name="time" value="{{old('time')}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-lime-300 focus:border-lime-300 block w-full p-2.5">
                </div>
                <!--Players-->
                <div>
                    <label for="players" class="block mb-2 text-sm font-medium text-gray-900">Number of players (write range with -)</label>
                    <input type="text" id="players" name="players" value="{{old('players')}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-lime-300 focus:border-lime-300 block w-full p-2.5">
                </div>
            </div>
            <!--Submit Button-->
            <button type="submit" class="px-5 py-3 shadow text-amber-600 bg-amber-300 hover:bg-amber-400 hover:text-amber-100 font-semibold rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center focus:outline-none">Add game</button>
            <!--Form END-->
        </form>
    </div>
</div>
